...
class ImageUtils {
    ...
    private var options: RequestOptions? = getRequestOptions(Color.TRANSPARENT)
    ...
    fun loadAvatar(context: Context, imageView: ImageView?, url: String?) {
        ...
        if (TextUtils.isEmpty(url)) {
            ...
            Glide.with(context)
                .load(url)
                .apply(
                    options!!.placeholder(ColorDrawable(Color.TRANSPARENT))
    ...
    fun loadImage(context: Context, imageView: ImageView?, drawableRes: Int?) {
        if (imageView == null)
            return
        loadImage(context, imageView, drawableRes ?: 0, Color.TRANSPARENT)
    }
